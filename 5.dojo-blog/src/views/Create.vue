...
<script>
//challenge
//  - add a submit eventhandler to the form
...
export default {
  setup() {
    ...
    const handleKeydown = () => {
      //checks if tags array incl the new tag value
      if (!tags.value.includes(tag.value)) {
        tag.value = tag.value.replace(/\s/, ""); //removes all whitespace (mellemrum)
...
/* pseudo element that injects info before the og element */
/* content is a empty string, it's to create the wonky orange block */
label::before {
  content: "";
  display: block;
  width: 100%;
